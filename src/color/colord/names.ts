...
import { ColorParseFunction, ColorParsers, RgbaColor } from './types.js';
 ...
 */
export function colorNamesPlugin(parsers: ColorParsers)
{
    ...
    Colord.prototype.toName = function toName(this: Colord, options: ColorConvertOptions)
    {
    ...
    // https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#transparent_keyword
        ...
        if (options?.closest)
        {
            ...
            if (!NAME_RGBA_STORE.length)
            {
                ...
                {
                    NAME_RGBA_STORE[name] = new Colord(NAME_HEX_STORE[name]).toRgb();
                }
            }
    ...
    const parseColorName: ColorParseFunction<string> = (input: string): RgbaColor | null =>
    {
    // the color names are case-insensitive according to CSS Color Level 3
        ...
        if (hex) return new Colord(hex).toRgb();
