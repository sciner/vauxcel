...
export class BaseImageResource extends Resource
{
    ...
    override upload(renderer: Renderer, baseTexture: BaseTexture, glTexture: GLTexture, source?: ImageSource): boolean
    {
        ...
        {
            gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, glTexture.format, glTexture.type, source);
        }
        else
        {
            glTexture.width = width;
            ...
            gl.texImage2D(baseTexture.target, 0, glTexture.internalFormat, glTexture.format, glTexture.type, source);
        }
