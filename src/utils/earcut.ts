...
export function earcut(data: ArrayLike<number>, holeIndices?: ArrayLike<number>, dim?: number): any[]
{
    dim = dim || 2;
...
function earcutLinked(ear: Node, triangles: Array<number>, dim: number,
    minX: number, minY: number, invSize: number, pass: number)
{
    if (!ear) return;
...
function isEar(ear: Node)
{
    ...
    var cx = c.x; var ay = a.y;
    var by = b.y; var cy = c.y;
...
function cureLocalIntersections(start: Node, triangles: Array<number>, dim: number)
{
    var p = start;
...
function splitEarcut(start: Node, triangles: Array<number>, dim: number, minX: number, minY: number, invSize: number)
{
...
function eliminateHoles(data: ArrayLike<number>, holeIndices: ArrayLike<number>, outerNode: any, dim: number)
{
    var queue = [];
    var i; var len;
    var start; var end;
...
export function earcutDiviation(data: ArrayLike<number>, holeIndices: ArrayLike<number>,
    dim: number, triangles: ArrayLike<any>)
{
    var hasHoles = holeIndices && holeIndices.length;
    var outerLen = hasHoles ? holeIndices[0] * dim : data.length;
...
export function earcutFlatten(data: ArrayLike<any>)
{
    var dim = data[0][0].length;
    var result = { vertices: [] as number[], holes: [] as number[], dimensions: dim };
    var holeIndex = 0;
